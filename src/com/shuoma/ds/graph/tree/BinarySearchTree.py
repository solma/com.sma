...
class BinarySearchTree:
    ...
    def insert(self, key, value):
        if not self._root:
            ...
            while cur:
                prev=cur
                if cur[_V]>value: cur=cur[_R]
                else:
                    if cur[_V]<value: cur=cur[_L]
                    else: return
            #print prev[_V], value
    ...
    def find(self,key):
        if not self._root:
            ...
            while cur:
                prev=cur
                if cur[_V] > key: cur=cur[_R]
                else:
                    if cur[_V] < key: cur=cur[_L]
                    else: return cur
    ...
    def print_(self):
        ...
        while depth<self._depth:
            ...
            while level==depth:
                if cur[_L] or cur[_R]:
                    ...
                    for i in range(len(children)):
                        if children[i]:
                            ...
                            child_line.append((queue[-1][2], str(queue[-1][0][_V])+","+str(queue[-1][0][_SUM])  ))
                            if i==0: bridge_line+=''.join([' ']*(child_pos-max(last_child_pos,last_father_pos)))+''.join(['-']*(pos+1-child_pos))
                            else:
                                if not children[0]: #left child is empty
                                    bridge_line+=''.join([' ']*(pos-max(last_child_pos,last_father_pos)-1))
                                    bridge_line+=' '
                ...
                del queue[0]
                if queue: cur, level, pos=queue[0]
                else: break
        ...
        for line in tree:
            if isinstance(line, str):
                ...
                for i in range(len(line)):
                    if i==0: print "%*s"%(line[i][0]-min_pos, line[i][1]),
                    else: print "%*s"%(line[i][0]-line[i-1][0], line[i][1]),
                print
        ...
        if not node: 
            ...
            if left_sum>-sys.maxsize and right_sum>-sys.maxsize:
                ...
                if left_sum>-sys.maxsize or right_sum>-sys.maxsize:
                    if left_sum>-sys.maxsize:     sum=left_sum+node[_V]
                    else:     sum=right_sum+node[_V]
                else: sum=node[_V]
...
N=3
print bst._printAllBinaryTree(N, 1, N)
