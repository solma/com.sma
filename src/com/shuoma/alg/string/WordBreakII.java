...
public class WordBreakII {
    ...
    public static ArrayList<String> wordBreak(String s, Set<String> dict) {
        ...
        for(int i=0;i<=s.length();i++){
        	...
        	for(String e: dict){
        		int en=e.length();
        		if( ((i>en&&dp.get(i-en).size()>0)||(i==en)) && s.substring(i-en,i).equals(e) && !s.substring(0, i-en).contains(e) ){
        			dp.get(i).add(i-en);
        		}
        	}
        }
    ...
    public static void helper(ArrayList<String> results,StringBuilder re, String s, Set<String> dict, ArrayList<ArrayList<Integer>> dp, int cur,int step){
        if(cur==0){
            results.add(re.toString());
            return;
        }
        for(int p:dp.get(cur)){
            if(step>0) re.insert(0," ");
            re.insert(0,s.substring(p,cur));
            helper(results,re,s,dict,dp,p,step+1);
            re.delete(0,cur-p);
            if(step!=0) re.deleteCharAt(0);
        }
    }
}
