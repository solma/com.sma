/**
  * given a set of {x: N}, where x is a character and N is the number x should be used. 
  * put all characters in one line such that for any x, no two occurrences of x are next to each other.
*/
import java.util.*;
public class AlignChars{
	...
	public void main(String characters, String numbers){
		...
		while(pq.size()>0){
			...
			if(nxtAvail.num>0)
				pq.add(nxtAvail);
			for(Counter c: collectors) pq.add(c);
			collectors.clear();
		}
