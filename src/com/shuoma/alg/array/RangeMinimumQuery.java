...
public class RangeMinimumQuery {
	...
	public void test(){
		int[] rdmArray={376, 47, 628, 540, 952, 142, 784, -454, 258, -316, -772, -886, 112, 901, 369, -484, 936, 978};
	...
	public int[] getMinimum(Interval[] intervals, int[] rdmArray, Solution alg){
		...
		case SPACE_EFFICIENT_DP:
			...
			for(int logNLen=0; logNLen<logNSize; logNLen++){
				for(int i=0;i<rdmArray.length;i++){
					int len= 1<<logNLen;
					mins[i][logNLen]= len==1?rdmArray[i]:Math.min(mins[i][logNLen-1], mins[Math.min(rdmArray.length-1, i+(len>>1) )][logNLen-1]);
					if(DEBUG) System.out.println("i="+i+" logNLen="+logNLen+" len="+len+" "+mins[i][logNLen]);
				}
			}
			...
			for(int i=0;i<ans.length;i++){
				Interval itvl=intervals[i];
				int logNLen=(int) Math.max(0, Math.ceil(Math.log(itvl.end-itvl.start+1)/Math.log(2))-1 ) ;
				ans[i]=Math.min(mins[(int)itvl.start][logNLen], mins[Math.max(0, (int)itvl.end+1-(1<<logNLen) )][logNLen]);
			}
			return ans;
		...
		case TREE_BASED:
			...
			ArrayList<ArrayInterval> nextLvL, curLvl;
			curLvl=leafLvl;
			do{
	...
	class ArrayInterval extends Interval{
		...
		public ArrayInterval(double start, double end, double min, ArrayInterval left, ArrayInterval right) {
			this(start, end, min);
			this.left=left;
			this.right=right;
		}
