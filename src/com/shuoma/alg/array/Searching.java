...
public class Searching {
	...
	public int[] generateRandomRotateArray() {
		int[] array = CommonUtils.genRandomArray();
		Arrays.sort(array);
		int n = array.length;
			...
			for (int i = 0; i < 10000; i++) {
				...
				case BIN_FIRST_SORTED:
					...
					if (idx != linearSearchFirstAppear(cpy, key))
						System.out.println(Arrays.toString(cpy) + " " + key
								+ " " + idx);
					break;
				...
				case BIN_ROTATED:
					...
					if (idx != linearSearchLastAppear(cpy, key)
							&& idx != linearSearchFirstAppear(cpy, key))
						System.out.println(Arrays.toString(cpy) + " " + key
								+ " " + idx);
					break;
				default:
	...
	int binarySearchFirstAppear(int[] a, int key) {
		...
		if (high >= a.length || a[high] != key)
			return high = -1; // this is the tricky line
		return high;
	}
