...
public class MaxPointsOnALine {
    ...
    public int maxPoints(Point[] points){
        ...
        int maxCnt=2;
        for(int i=0;i<n;i++){
            slopeCnt=new HashMap<Double, Integer>();
            int dup=1;
            for(int j=i+1;j<n;j++){
                double slope;
                ...
                else slope=(points[j].y-points[i].y+0.0)/(points[j].x-points[i].x)+0.0;//problem of -0.0
                if(!slopeCnt.containsKey(slope)) slopeCnt.put(slope, 0);
