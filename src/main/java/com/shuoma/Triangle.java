...
public class Triangle {
    ...
    public int minimumTotal(ArrayList<ArrayList<Integer>> triangle) {
        ...
        if(n==0) return ret;
        if(n==1){
            if(triangle.get(0).size()==0) return ret;
            else return triangle.get(0).get(0);
        } 
        ...
        for(int i=1;i<triangle.size();i++){
            ...
            for(int j=0;j<row.size();j++){
                if(j==0) row.set(j,row.get(j)+prevRow.get(j));
                else if(j==row.size()-1) row.set(j, row.get(j)+prevRow.get(j-1));
                else row.set(j, row.get(j)+Math.min(prevRow.get(j), prevRow.get(j-1) ));
            }
