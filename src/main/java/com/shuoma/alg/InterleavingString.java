...
public class InterleavingString {
  ...
  public boolean isInterleave(String s1, String s2, String s3, int j, int k) {
    for (int i = j + k; i < s3.length(); i++) {
      if (j < s1.length() && s1.charAt(j) != s3.charAt(i) && k < s2.length() && s2.charAt(k) != s3
          .charAt(i))
        return false;
      if (j < s1.length() && s1.charAt(j) == s3.charAt(i) && k < s2.length() && s2.charAt(k) == s3
          .charAt(i)) {
        ...
        if (j < s1.length() && s1.charAt(j) == s3.charAt(i))
          return isInterleave(s1, s2, s3, j + 1, k);
        if (k < s2.length() && s2.charAt(k) == s3.charAt(i))
          return isInterleave(s1, s2, s3, j, k + 1);
        return false;
      }
    }
    return true;
