...
public class BinaryEncoding{
    ...
    public class SymbolTreeNode implements Comparable{
        public Symbol node;
        ...
        public int compareTo(Object other){
            SymbolTreeNode stn=( SymbolTreeNode)other;
            return node.compareTo(stn.node);
        }
        public String toString(){
    ...
    public void HuffmanEncoding(Symbol[] syms){
        PriorityQueue<SymbolTreeNode> pq=new PriorityQueue<SymbolTreeNode>();
        for(int i=0;i<syms.length;i++) pq.add(new SymbolTreeNode(syms[i]) );
        ...
        while(!pq.isEmpty()){
            ...
            SymbolTreeNode newNode=new SymbolTreeNode(new Symbol(min1.node.sym+min2.node.sym,min1.node.prob+min2.node.prob));
            newNode.left=min1;
            newNode.right=min2;
    ...
    private int partition(Symbol[] syms, int l, int r){
        ...
        l--; r++;
        while(l+1!=r){
            if(sumLeft<sumRight)  sumLeft+=syms[++l].prob;
            else sumRight+=syms[--r].prob;
        }
