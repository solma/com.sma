...
public class MultipleChoicesAnsweringStrategy {
  ...
  public static void main(String[] args) {
    ...
    for (int i = 0; i < times; i++) {
      ...
      double cycleRate = ins.getCorrectPercentage(ins.genAnswersByFillingCyclingChoices(),
          groundTruthAns);
      //System.out.println("cycle: " + cycleRate);

      double singleAvgRate = 0, singleMaxRate = 0;
      for (int j = 0; j < ins.k; j++) {
        double rate = ins.getCorrectPercentage(groundTruthAns, ins.genAnswersByFillingSingleChoice(j));
        singleAvgRate += rate;
    ...
    System.out.println("cycle wins : " + cycleWins + " draws: " + draws
        + " single wins: " + (times - draws - cycleWins));
  }
