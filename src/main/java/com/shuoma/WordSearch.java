...
public class WordSearch {
    ...
    public boolean exist(char[][] board, String word) {
        ...
        for(int i = 0; i < board.length; i++)
            for(int j = 0; j < board[0].length; j++){
            	...
            	if(board[i][j]==word.charAt(0)&& BFS(board, i, j, word, visited)) return true;
            }
        return false;
    }
    	...
    	while(!curLvl.isEmpty()){
			...
			while(!curLvl.isEmpty()){
    			...
    			if(x>0&&!v[x-1][y]&&b[x-1][y]==word.charAt(idx)){
    				nextLvl.add(new Node(x-1, y)); v[x-1][y]=true;
    			}
    			if(y>0&&!v[x][y-1]&&b[x][y-1]==word.charAt(idx)){
    				nextLvl.add(new Node(x, y-1)); v[x][y-1]=true;
    			}
        ...
        for(int i=0;i<board.length;i++)
            for(int j=0; j<board[0].length;j++)
                if(board[i][j]==word.charAt(0)){
                    visited[i][j]=true;
                    if( existRecursion(board, word, dirs, i, j, 1, visited ) )   return true;
                }
        return false;
    }
    ...
    public boolean existRecursion(char[][] board, String word,int[][]dirs, int i, int j, int idx, boolean[][] visited){
        ...
        for(int k=0;k<dirs.length;k++){
            if( (i+dirs[k][0]>=0)&&(i+dirs[k][0]<board.length)&&(j+dirs[k][1]>=0)&&(j+dirs[k][1]<board[0].length)&& !visited[i+dirs[k][0]][j+dirs[k][1]] && board[i+dirs[k][0]][j+dirs[k][1]]==word.charAt(idx) ){
                visited[i+dirs[k][0]][j+dirs[k][1]]=true;
                if(existRecursion(board, word, dirs, i+dirs[k][0], j+dirs[k][1], idx+1, visited))  return true;
