...
public class SearchForARange {
   ...
   public int[] searchRange(int[] A, int target) {
    ...
    public int[] searchRangeNotLogN(int[] A, int target) {
        ...
        while(low<=high){
            med=low+(high-low)/2;
            if(A[med]==target) break;
            else{
                if(A[med]>target) high=med-1;
                else low=med+1;
            }            
        }
        if(low>high) return new int[]{-1, -1};
            ...
            int tmp=med;
            while(med>=0 && A[med]==target) med-=1;
            int[] ret=new int[2];
            ret[0]=med+1;
            med=tmp;
            while(med<A.length&&A[med]==target) med+=1;
            ret[1]=med-1;
            return ret;
        }
    }
    ...
    public int[] searchRangeLogN(int[] A, int target) {
        ...
        if (right >= A.length || A[right] != target)  right = -1;        
    ...
    int BestBinarySearch(int array[], int n, int v)
    {
        ...
        {
            middle = left + (right-left)/2;
            if (array[middle] < v) left = middle;
            else right = middle;            
        }
        if (right >= n || array[right] != v)  right = -1;        
        return right;
    }
