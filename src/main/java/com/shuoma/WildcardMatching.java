...
public class WildcardMatching {
    public static void main(String[] args){
        new WildcardMatching().main();
    }
    
    public void main(){
        System.out.println(isMatch("aaabbbaabaaaaababaabaaabbabbbbbbbbaabababbabbbaaaab", "a*******b"));
    }
    ...
    public boolean isMatch(String s, String p) {
        ...
        while (pid < p.length()) {
            ...
            System.out.println("pi="+pid+" "+p.charAt(pid)+" "+" firstMatch="+firstMatch+"  "+Arrays.toString(matches));
    ...
    public boolean isMatch(String s, String p, int sIdx, int pIdx){
        ...
        if(y!='*'){
            ...
            for(int i=sIdx; i<=s.length(); i++){
                if(isMatch(s,p, i, pIdx+1)) return true;
            }
            return false;
        }
    }
