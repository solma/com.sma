...
public class MinimumWindowSubstring{
    ...
    public String minWindow(String S, String T) {
        ...
        for(end=0;end<nS;end++){
            ...
            if(hasFound[(int)S.charAt(end)]<=needToFind[(int)S.charAt(end)]) cnt++;
            
            if(cnt==nT){
                //System.out.println(S.charAt(end)+" "+needToFind[(int)S.charAt(end)]);
                while(needToFind[(int)S.charAt(begin)]==0 || hasFound[(int)S.charAt(begin)]>needToFind[(int)S.charAt(begin)]){
                    if(hasFound[(int)S.charAt(begin)]>needToFind[(int)S.charAt(begin)]) hasFound[(int)S.charAt(begin)]--;
                    begin++;
                }
                if(end-begin<min){
                    min=end-begin;
        ...
        if(minL<=minR) return S.substring(minL, minR+1);
        else return "";
    }
}