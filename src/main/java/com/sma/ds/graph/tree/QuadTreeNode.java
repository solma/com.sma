...
public class QuadTreeNode<T extends Rectangle> {
  ...
  private void split() {
    ...
    nodes[3] = new QuadTreeNode(level + 1,
  ...
  public void insert(T obj) {
    ...
    if (objects.size() > MAX_OBJECTS && level < MAX_LEVELS) {
      ...
      if (objects.size() > MAX_OBJECTS)
        MAX_OBJECTS = Integer.MAX_VALUE;  //only split() and put in once.
    }
  }
  ...
  public List retrieve(List<T> collisions, T obj) {
    if (isSplit) {
      int index = getIndex(obj);
      if (index != -1)
        nodes[index].retrieve(collisions, obj);
      else {
        for (int i = 0; i < 4; i++)
          nodes[i].retrieve(collisions, obj);
      }
    }
