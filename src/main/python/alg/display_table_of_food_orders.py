...
More specifically orders[i]=[customerName, tableNumber,foodItem] where customerName is the name of the customer,
tableNumber is the table customer sit at, and foodItem is the item customer orders.
...
each food item in alphabetical order. The first row should be a header whose first column is “Table”,
...
class Solution(object):
  def displayTable(self, orders):
    ...
    ret = [["Table"] + sorted_items]
    for table in sorted(tables.keys()):
      table_order = [str(table)]
      for item in sorted_items:
        if item not in tables[table]:
          table_order.append('0')
        else:
          table_order.append(str(tables[table][item]))
