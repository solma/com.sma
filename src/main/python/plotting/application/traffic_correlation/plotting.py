...
def plot_multiple_line_for_each_of_the_dichotomy(paras, xAxis, yAxis, yError, yProbeSpdStd, title, lane_based=False, express_or_hov_lane=False, tmc=None):
    ...
    dichotomy=paras[1] 
    if not dichotomy:
        print 'dichotomy has to be True' 
        return
...
def plot_scatter(paras, xAxis, yAxis, yError, yProbeSpdStd, title):
    ...
    print paras, [len(s) for s in xAxis],[len(s) for s in yAxis],[len(s) for s in yError], title, "\n"
    ...
    for dich in dichotomy:
        for yAxisChoice in plotting_error:            
            for ver in condition.keys():
                ...
                xAxis, yAxis, yError, yProbeSpdStd, title=read_data(paras, lane_based=use_no_of_lane, express_or_hov_lane=use_express_and_hov_lane,  tmc=tmc_value)
                #plot_scatter(paras, xAxis, yAxis, yError, yProbeSpdStd, title)
                plot_multiple_line_for_each_of_the_dichotomy(paras, xAxis, yAxis, yError, yProbeSpdStd, title, lane_based=use_no_of_lane, express_or_hov_lane=use_express_and_hov_lane, tmc=tmc_value)
