...
def draw_activity_plot():
  ...
  for i in range(NUM_ACTIVITY_TYPE):
    vlines.append(
      vax.vlines(timestamps[i], [confidence_threshold], confidences[i], colors=colors[i]))  # linestyles=line_styles[i])
  ...
  if ground_truth_set:
    for i in range(NUM_ACTIVITY_TYPE):
      for period in ground_truth_time_duration[i]:  # not empty
        bars.append(plt.bar(period[0], 100 - (confidence_threshold - ybottom_extra), width=period[1] - period[0],
                            bottom=(confidence_threshold - ybottom_extra), color=ground_truth_bg_color[i]))
...
def plot_selected_fields(file_name
  ...
  vline_top_value = [[] for i in range(
    len(idx_of_plotting_fields))]  # maximum value of this field during a time unit (e.g. a second)
  vline_bottom_value = [[] for i in range(
    len(idx_of_plotting_fields))]  # minimum value of this field during a time unit (e.g. a second)
  ...
  print
  ground_truth_idx_range
  print
  "# of values on x axis= ", len(x_axis_idx), " len of vline_top_value= ", len(vline_top_value[-1])
    ...
    if ground_truth_set:
      for gt_i in range(len(ground_truth_idx_range)):
        for period in ground_truth_idx_range[gt_i]:  # not empty
          bars.append(plt.bar(period[0], height=y_axis_span[2] + 2 * y_axis_extra_padding, width=period[1] - period[0],
                              bottom=y_axis_span[0] - y_axis_extra_padding, color=ground_truth_bg_color[gt_i]))
          if period[1] - period[0] > x_axis_span[2] / 10:  # only if the period is wide enough to accommodate the label
            vax.annotate(activity_type[gt_i].replace("_", "-"),
                         xy=(period[0] + (period[1] - period[0]) / 10, y_axis_span[0] - y_axis_extra_padding),
                         fontsize=10)
...
def plot_detected_events(date):
  ...
  li = [66538, 66538, 66602, 66650, 66651, 66651, 66651, 66651, 66651, 66652, 66717, 66719, 66719, 66719, 66719, 66719,
        66720, 66720, 66720, 66724, 66724, 66724, 66725, 66725, 66725, 66725, 66725, 66726, 66726, 66726, 66753, 66753,
        66754, 66755, 66755, 66755, 66755, 66755, 66755, 66755, 66756, 66758, 66758, 66758, 66758, 66758, 66758, 66758,
        66758, 66759, 66761, 66761, 66768, 66768, 66771, 66771, 66778, 66778, 66778, 66778, 66778, 66778, 66779, 66779,
        66783, 66793, 66793, 66793, 66794, 66794, 66794, 66794, 66795, 66795, 66795, 66795, 66795, 66795, 66795, 66796,
        66796, 66796, 66796, 66797, 66798, 66798, 66799, 66799, 66815, 66816, 66817, 66824, 66824, 66824, 66824, 66824,
        66824, 66824, 66825, 66825, 66826, 66826, 66847, 66848, 66848, 66848, 66848, 66849, 66850, 66850, 66850, 66851,
        66851, 66851, 66851, 66851, 66851, 66852, 66852, 66852, 66852, 66852, 66853, 66853, 66853, 66853, 66853, 66853,
        66853, 66854, 66854, 66855, 66855, 66855, 66855, 66856, 66856, 66856, 66856, 66856, 66858, 66858, 66858, 66858,
        66858, 66859, 66859, 66859, 66859, 66859, 66859, 66861, 66862, 66862, 66862, 66863, 66863, 66865, 66866, 66866,
        66866, 66866, 66866, 66867, 66867, 66868, 66868, 66869, 66869, 66869, 66869, 66870, 66870, 66871, 66873, 66873,
        66874, 66874, 66874, 66875, 66931, 66931, 66932, 66932, 66932, 66932, 66933, 66933, 66942, 66942, 66978, 66978,
        66978, 66978, 67018, 67018, 67018, 67019, 67020, 67021, 67021, 67021, 67021, 67021, 67040, 67041, 67349, 67350,
        66856, 66856, 66856, 66858, 66858, 66858, 66858, 66858, 66858, 66859, 66859, 66859, 66859, 66859, 66861, 66862,
        66862, 66862, 66863, 66863, 66865, 66866, 66866, 66866, 66866, 66866, 66867, 66867, 66868, 66868, 66869, 66869,
        66869, 66869, 66870, 66870, 66871, 66873, 66873, 66874, 66874, 66874, 66875, 66931, 66931, 66932, 66932, 66932,
        66932, 66933, 66933, 66942, 66942, 66978, 66978, 66978, 66978, 67018, 67018, 67018, 67018, 67020, 67021, 67021,
        67021, 67021, 67021, 67040, 67041, 67349, 67349, 67349, 67350, 67350, 67350, 67351, 67351, 67351, 67392, 67394,
        67394, 67394, 67395, 67396, 67396, 67396, 67396, 67398, 67398, 67401, 67401, 67402, 67420, 67421, 67421, 67422,
        67424, 67426, 67471, 67481, 67482, 67482, 67484, 67515, 67515, 67515, 67515, 67515, 67518, 67524, 67527, 67533,
        67533, 67534, 67535, 67678, 67679, 67679, 67679, 67679, 67679, 67686, 67686, 67725, 67726, 67726, 67726, 67726,
        67727, 67727, 67727, 67727, 67727, 67727, 67727, 67728, 67728, 67728, 67729, 67729, 67729, 67730, 67731, 67731,
        67732, 67733, 67733, 67733, 67733, 67734, 67734, 67734, 67735, 67735, 67735, 67735, 67736, 67739, 67739, 67739,
        67740, 67740, 67740, 67741, 67741, 67741, 67741, 67741, 67741, 67742, 67742, 67744, 67745, 67745, 67745, 67766,
        67766, 67766, 67766, 67766, 67768, 67769, 67769, 67769, 67776, 67778, 67778, 67780, 67789, 67800, 67800, 67800,
        67800, 67800, 67800, 67800, 67801, 67801, 67850, 67851, 67869, 67917, 68119, 68126, 68127, 68145, 68162, 68167,
        68342, 68364, 68397, 68399, 68407, 68409, 68413, 68432, 68433, 68433, 68433, 68434, 68434, 68434, 68434, 68434,
        68434, 68435, 68480, 68485, 68485, 68493, 68493, 68493, 68493, 68493, 68493, 68494, 68494, 68509, 68520, 68520,
        68520, 68520, 68521, 68521, 68521, 68521, 68522, 68524, 68539, 68539, 68541, 68542, 68542, 68560, 68688, 68768,
        68768, 68769, 68769, 68843, 68846, 68846, 68851, 68878, 68884, 68884, 68893, 68894, 68894, 68898, 68898, 68919,
        68923, 68930, 68930, 68933, 68933, 68951, 68952, 68952, 68952, 68954, 68955, 68956, 68956, 68956, 68958, 68958,
        68958, 68961, 68978, 68983, 68983, 68983, 68986, 68986, 68986, 69001, 69002, 69002, 69004, 69008, 69008, 69008,
        69008, 69008, 69144, 69146, 69146, 69147, 69148, 69210, 69214]  # 09-11-accel-weka
...
def draw_accel_raw_files():
  ...
  file_names = [['08_2355', '08_2462', '08_2486'],
                ['08_2311', '08_2710', '08_295', '09_211', '09_191', '08_283', '10_241', '10_242', '10_251', '10_245']]
  file_labeled = 1
  ...
  for n in file_names[file_labeled][-1:]:
    ...
    plot_selected_fields(file_name
                         ...
                         , name_of_plotting_fields=['acceleration of X Axis', 'acceleration of Y Axis',
                                                    'acceleration of Z Axis', 'acceleration']
...
def draw_feature_data():
  ...
  plot_selected_fields(
    'E:/workspace/android/UPDetector/data/accelerometer/change_in_variance/' + pos + '/all/ACCELEROMETER_FEATURE_' + date + '.arff'
    , idx_of_plotting_fields=[5]
